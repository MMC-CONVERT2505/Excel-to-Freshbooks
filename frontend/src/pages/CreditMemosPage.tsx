...
import { getCreditMemos } from '../lib/api';
// @ts-ignore
import XLSXStyle from 'xlsx-js-style';

const COLUMNS = [
  { key: 'creditid',       label: 'ID' },
...
function downloadExcel(rows: any[]) {
  ...
  ws['!cols'] = [6,14,12,10,24,14,14,12,10,12,28].map(wch => ({ wch }));
  ws['!rows'] = [{ hpt: 22 }];
  ...
  XLSXStyle.utils.book_append_sheet(wb, ws, 'Credit Memos');
  XLSXStyle.writeFile(wb, 'freshbooks_credit_memos.xlsx');
}
...
export default function CreditMemosPage() {
  ...
  function load() {
    ...
    getCreditMemos()
      .then(d => { setRows(d.response.result.credit_notes); setLoading(false); })
    ...
    <div className="proj-wrap">
      <div className="proj-header">
        <div className="proj-header__text">
          <h2 className="proj-header__title">Credit Memos</h2>
          <p className="proj-header__sub">
            {loading ? 'Loading…' : `${rows.length} credit memo${rows.length !== 1 ? 's' : ''} in this account`}
          </p>
        </div>
      ...
      {loading && <div className="proj-state"><div className="proj-spinner" /><span>Fetching credit memos…</span></div>}

      {!loading && error && (
        <div className="proj-state proj-state--error">
          <b>Failed to load credit memos</b><span>{error}</span>
          <button className="btn btn--ghost btn--sm" onClick={load}>Try again</button>
        </div>
      )}

      {!loading && !error && rows.length === 0 && (
        <div className="proj-state"><b>No credit memos found</b><span>This FreshBooks account has no credit memos yet.</span></div>
      )}
      ...
      {!loading && !error && rows.length > 0 && (
        <div className="card" style={{ overflowX: 'auto' }}>
          <table className="hist-table" style={{ minWidth: 700 }}>
            <thead>
              <tr>
                {COLUMNS.filter(c => ['credit_number','status','organization','lname','amount_amount','currency_code','create_date'].includes(c.key)).map(c => (
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={i}>
                  {['credit_number','status','organization','lname','amount_amount','currency_code','create_date'].map(k => (
